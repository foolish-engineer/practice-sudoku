...
function shuffleBoard(board) {
	...
	for (let block = 0; block < 3; block++) {
		...
		for (let i = rows.length - 1; i > 0; i--) {
			...
			[newBoard[rows[i]], newBoard[rows[j]]] = [
				newBoard[rows[j]],
				newBoard[rows[i]],
			];
		}
	...
	for (let block = 0; block < 3; block++) {
		...
		for (let i = cols.length - 1; i > 0; i--) {
			...
			for (let row = 0; row < 9; row++) {
				[newBoard[row][cols[i]], newBoard[row][cols[j]]] = [
					newBoard[row][cols[j]],
					newBoard[row][cols[i]],
				];
			}
...
function App() {
	...
	return (
		<div className="sudoku-app">
			...
			>
				<button type="button" onClick={() => handleNewSudoku("easy")}>
					New Easy
				</button>
				<button type="button" onClick={() => handleNewSudoku("medium")}>
					New Medium
				</button>
				<button type="button" onClick={() => handleNewSudoku("hard")}>
					New Hard
				</button>
			</div>
