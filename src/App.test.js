...
function isValidSolution(board) {
	...
	// Check rows
	for (let r = 0; r < 9; r++) {
		const row = new Set(board[r].map(Number));
		if (row.size !== 9 || ![...row].every((v) => expected.has(v))) return false;
	}

	// Check columns
	for (let c = 0; c < 9; c++) {
		const col = new Set(board.map((row) => Number(row[c])));
	...
	for (let br = 0; br < 3; br++) {
		for (let bc = 0; bc < 3; bc++) {
			...
			for (let r = 0; r < 3; r++)
				for (let c = 0; c < 3; c++)
					box.add(Number(board[br * 3 + r][bc * 3 + c]));
...
describe("generatePuzzle", () => {
	...
	it("medium: has fewer or equal clues than easy (harder difficulty)", () => {
		const easyClues = countClues(generatePuzzle("easy").puzzleBoard);
		const medClues = countClues(generatePuzzle("medium").puzzleBoard);
		expect(medClues).toBeLessThanOrEqual(easyClues);
	}, PUZZLE_TIMEOUT);

	it("hard: has fewer or equal clues than medium (hardest difficulty)", () => {
		const medClues = countClues(generatePuzzle("medium").puzzleBoard);
		const hardClues = countClues(generatePuzzle("hard").puzzleBoard);
		expect(hardClues).toBeLessThanOrEqual(medClues);
	}, PUZZLE_TIMEOUT);
